...
export function generatePrompts(
  ...
  const emotionalTriggers = patterns.filter(
    (p) => p.pattern_type === "emotional_trigger"
  );
  ...
  const correctionIds = new Set(
    entries.filter((e) => e.event_type === "correction").map((e) => e.id)
  );
  const assumptionsToTest: string[] = [];
  for (const e of recentWithAssumptions) {
    if (correctionIds.has(e.id)) continue;
    for (const a of e.assumptions ?? []) {
      if (assumptionsToTest.length >= 3) break;
  ...
  if (latest) {
    const trigger = emotionalTriggers.find(
      (p) => p.description.includes(latest.emotional_state)
    );
    if (trigger) {
      out.push({
        id: generateId(),

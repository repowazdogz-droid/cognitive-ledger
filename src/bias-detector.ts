...
import type { BiasDetection, BiasType, LedgerEntry } from "./types";
...
export function detectBiases(
  ...
  if (entry.stated_confidence >= 0.85) {
    const highConfPast = history.filter(
      (e) => e.stated_confidence >= 0.85 && e.outcome !== undefined
    );
    if (highConfPast.length >= 3) {
      const avgAccuracy =
        highConfPast.reduce((s, e) => s + (e.outcome!.accuracy ?? 0), 0) /
        highConfPast.length;
      if (avgAccuracy < 0.7) {
        out.push({
          type: "overconfidence",
  ...
  if (
    (entry.alternatives_considered?.length ?? 0) === 0 &&
  ...
  if (sameDomain.length >= 3) {
    ...
    const allNearFirst = recent.every(
      (e) => Math.abs(e.stated_confidence - firstConf) <= 0.1
    );
  ...
  if (assumptions.length > 0) {
    const withRecent = assumptions.filter((a) =>
      RECENT_TIME_WORDS.some((w) => lower(a).includes(w))
    );
  ...
  const hasCorrelation = CORRELATION_WORDS.some((w) =>
    lower(evidenceText).includes(w)
  );
  if (hasCausal && evidenceText.length > 0 && hasCorrelation) {
    out.push({
      ...
      confidence: 0.7,
      evidence:
        "Content uses causal language while evidence uses correlation language.",
  ...
  const decisionsInDomain = history.filter(
    (e) => e.domain === entry.domain && e.event_type === "decision"
  );
  const last5 = decisionsInDomain.slice(-5);
  if (last5.length >= 5) {
    const withStatusQuo = last5.filter((e) =>
      STATUS_QUO_WORDS.some((w) => lower(e.content).includes(w))
    );

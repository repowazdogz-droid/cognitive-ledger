...
exports.OPERATION_RULE_SCHEMAS = void 0;
exports.buildRuleSignature = buildRuleSignature;
exports.validateReasoningStep = validateReasoningStep;
exports.validateReasoningSteps = validateReasoningSteps;
exports.normalizeReasoningSteps = normalizeReasoningSteps;
const VALUE_TYPES = [
    "text",
    "boolean",
    "number",
...
exports.OPERATION_RULE_SCHEMAS = OPERATION_RULE_SCHEMAS;
function buildRuleSignature(step) {
    return `${step.rule_schema}:${step.operation_type}(${step.input_type.join(",")})->${step.output_type}`;
}
function isKnownValueType(value) {
...
function validateReasoningStep(step) {
    const errors = [];
    if (!step.id)
    ...
    if (!Array.isArray(step.input_type))
        errors.push("input_type must be an array");
    if (!Array.isArray(step.depends_on))
        errors.push("depends_on must be an array");
    if (!Array.isArray(step.evidence_refs))
        errors.push("evidence_refs must be an array");
    if (!step.rule_schema)
        errors.push("rule_schema is required");
    ...
    if (!rule) {
        ...
        if (step.rule_schema !== rule.rule_schema) {
            errors.push(`rule_schema must be ${rule.rule_schema} for ${step.operation_type}`);
        }
        if ((step.input_type?.length ?? 0) < rule.min_inputs) {
            errors.push(`${step.operation_type} requires at least ${rule.min_inputs} input type(s)`);
        }
        if (!rule.output_types.includes(step.output_type)) {
            errors.push(`${step.operation_type} cannot output ${step.output_type}`);
        }
    }
...
function validateReasoningSteps(steps) {
    const errors = [];
    const ids = new Set();
    ...
    for (const step of steps) {
        ...
        if (ids.has(step.id))
            errors.push(`${step.id}: duplicate id`);
        ids.add(step.id);
        if (indexes.has(step.index))
            errors.push(`${step.id}: duplicate index`);

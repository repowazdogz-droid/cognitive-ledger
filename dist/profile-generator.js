...
function generateProfile(personId, entries, patterns) {
    ...
    const domains_active = Array.from(new Set(entries.map((e) => e.domain)));
    const strengthPatterns = patterns.filter((p) => p.pattern_type === "strength");
    const strengths = strengthPatterns.map((p) => p.description.replace(/^Strength in (\w+).*/, "$1"));
    const biasRecurring = patterns.filter((p) => p.pattern_type === "bias_recurring");
    const recurring_biases = biasRecurring.map((p) => {
        const typeMatch = p.description.match(/Recurring (\w+)/);
        const type = (typeMatch?.[1] ?? "confirmation");
        return {
            type,
    ...
    if (withOutcomes.length > 0) {
        const sumDiff = withOutcomes.reduce((s, e) => s + Math.abs(e.stated_confidence - (e.outcome.accuracy ?? 0)), 0);
    ...
    for (const d of domains_active) {
        ...
        const sumDiff = inDomain.reduce((s, e) => s + Math.abs(e.stated_confidence - (e.outcome.accuracy ?? 0)), 0);
    ...
    for (const e of withOutcomes) {
        ...
        cur.total++;
        if ((e.outcome.accuracy ?? 0) >= 0.7)
            cur.good++;
        stateOutcomes.set(e.emotional_state, cur);
    }
    const best_decisions_made_when = [];
    const worst_decisions_made_when = [];
    for (const [state, data] of stateOutcomes) {
        if (data.total < 3)
            continue;
        if (data.good / data.total >= 0.7)
            best_decisions_made_when.push(state);
        if (data.good / data.total <= 0.3)
            worst_decisions_made_when.push(state);
    }
    const half = Math.floor(withOutcomes.length / 2);
    let growth_trajectory = 0;
    if (withOutcomes.length >= 2 && half >= 1) {
        const firstAvg = withOutcomes
            .slice(0, half)
            .reduce((s, e) => s + (e.outcome.accuracy ?? 0), 0) / half;
        const secondAvg = withOutcomes
            .slice(half)
    ...
    const hash = (0, hash_1.sha256)(payload);
    return {
        person_id: personId,
        generated_at,

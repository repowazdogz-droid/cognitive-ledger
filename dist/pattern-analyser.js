...
exports.analysePatterns = analysePatterns;
const hash_1 = require("./hash");
function clamp01(x) {
    return Math.max(0, Math.min(1, x));
}
function mergeOrAdd(patterns, key, create, update) {
...
function analysePatterns(entries, existingPatterns) {
    const byKey = new Map();
    ...
    for (const [domain, data] of outcomesByDomain) {
        if (data.total < 5)
            continue;
        if (data.accurate / data.total < 0.8)
            continue;
    ...
    for (const e of entries) {
        ...
        const withOutcome = entries.filter((x) => x.domain === domain && x.outcome != null);
        if (withOutcome.length < 3)
            continue;
        const avgAccuracy = withOutcome.reduce((s, x) => s + x.outcome.accuracy, 0) /
            withOutcome.length;
        const avgConf = withOutcome.reduce((s, x) => s + x.stated_confidence, 0) /
            withOutcome.length;
    ...
    for (const [state, data] of byState) {
        if (data.total < 3)
            continue;
        if (data.total - data.accurate < data.total * 0.7)
            continue;
